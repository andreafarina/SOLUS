...
% Jconv = toastJacobianTimedomain_conv_time(hMesh,hBasis,qvec,mvec, dmask, mua0, mus0, ref,dt,nstep,twin,irf);
J = toastJacobianTimedomain_INPROGRESS(hMesh,hBasis,qvec,mvec, ...
...
%% plot
for i = 1:50:nstep
mua_d = -J(i,1:slen);%* vscale * dt * 1e3;% / cm;
mua_e = -Je(i,1:slen);
mus_d = -J(i,slen+1:end);%* vscale * dt * 1e3;
...
subplot(2,3,1);imagesc(img_d,[mn mx]); axis equal tight; title('direct Fourier'); colorbar

img_e = reshape(hBasis.Map('S->B',mua_e),grd);
%subplot(1,3,2);imagesc(img,[mn mx]); axis equal tight; title('explicit'); colorbar
subplot(2,3,2);imagesc(img_e,[mn mx]); axis equal tight; title('explicit'); colorbar

ratio = img_d./img_e;
%img = reshape(hBasis.Map,ratio),grd);
subplot(2,3,3);imagesc(ratio);axis equal tight;colorbar;title('ratio Fourier/explicit')

mn = min([mus_d mus_e]);% mua_c]);
mx = max([mus_d mus_e]);% mua_c]);
img_d = reshape(hBasis.Map('S->B',mus_d),grd);
%subplot(1,3,1);imagesc(img,[mn mx]); axis equal tight; title('direct'); colorbar
subplot(2,3,4);imagesc(img_d,[mn mx]); axis equal tight; title('direct Fourier'); colorbar

img_e = reshape(hBasis.Map('S->B',mus_e),grd);
%subplot(1,3,2);imagesc(img,[mn mx]); axis equal tight; title('explicit'); colorbar
subplot(2,3,5);imagesc(img_e,[mn mx]); axis equal tight; title('explicit'); colorbar

ratio = img_d./img_e;
%img = reshape(hBasis.Map,ratio),grd);
subplot(2,3,6);imagesc(ratio);axis equal tight;colorbar;title('ratio Fourier/explicit')
