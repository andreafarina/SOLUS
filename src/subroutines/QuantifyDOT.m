...
for nlambda = 1:REC.radiometry.nL
    for n_mu = 1:n_mu
        ...
        tmp_mu = mu(idx_incl);
        tmp_bin_idx = zeros(size(mu));
        tmp_bin_idx(idx_incl) = 1;
...
%         end
        ...
        for i_field = {'C','C_vol', 'CNR','CNR_vol', 'displ',...
                'brd','brdRMS', 'acc', 'acc_vol', 'ValIn','ValOut'}
...
if contains(lower(REC.solver.type),'spectral')
    ...
    if n_mu == 1 || (numel(REC.opt.hete1.type) == 2)
        for ic = 1:REC.spe.nCromo
            ...
            if ~isempty(REC.opt.Conc)
                target_mu = REC.opt.Conc(:,:,:,ic);
            else
                target_mu = REC.opt.Mua(:,:,:,1);
            end
            if ic == 1
                coeff = 'Hb';
            elseif ic == 2
