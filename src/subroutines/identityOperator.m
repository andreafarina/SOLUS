function [Dy, Dx, Dz, Lap] = identityOperator(volume, h, c, BC, mask)
...
Dy_block =    1/(h(1))*spdiags(ones(ny,       1),   0,     ny,    ny);
...
%              + spdiags(ones(ny*nx   , 1),   ny,    ny*nx, ny*nx);  
  Dx_block =  1/(h(2))*spdiags(ones(ny*nx,    1),   -ny,    ny*nx, ny*nx);  
...
%              + spdiags(ones(n,        1),   ny*nx, n,     n);
  Dz       =  1/(h(3))*spdiags(ones(n,        1),   -ny*nx,     n,     n) ;
